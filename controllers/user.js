...
async function login(req, res) {
    ...
    let cursor = collection.find({ $or: [{ $and: [{ email: username }, { password: password }, {isDeleted: {$ne: true} }] }, { $and: [{ number: username }, { password: password },{isDeleted: {$ne: true} }] }] })
...
async function register(req, res) {
    ...
    let cursor = collection.find({ $and: [{ $or: [{ email: email }, { number: number }], isDeleted: {$ne: true} }] })
    let user = await cursor.toArray()
    if (user.length) {
