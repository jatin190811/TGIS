...
async function createBridal(req, res) {
    let name, price, isFeatured, specifications, description,  ameneties, execuisite, facts, contact_details, tags, type, sub_cat, address;
...
async function updateBridal(req, res) {
    ...
    console.log({ _id: ObjectId(req.body.pid) }, {
        name,
        price,
        isFeatured,
        specifications,
...
async function imageUpload(req, res) {
    ...
    if (req.files && req.files.length) {
        images = req.files.map(i => {
            const newName = 'makeup/'+makeid(14) + "." + i.filename.split('.').pop()
    ...
    if (result.modifiedCount) {
        return res.json({ status: 'success', message: 'Makeup Images successfully Updated', data: {} })
    } else {
        return res.json({ status: 'error', error: '009', message: 'Something went wrong' })
    }
...
async function imageDelete(req, res) {
    ...
    try {
        fs.unlinkSync('/public/makeup/' + deletedImage)
        if (result.modifiedCount) {
            return res.json({ status: 'success', message: 'Makeup Images successfully Updated', data: {} })
        } else {
            return res.json({ status: 'error', error: '009', message: 'Something went wrong' })
...
async function videoUpload(req, res) {
    ...
    if (req.files && req.files.length) {
        videos = req.files.map(i => {
            const newName = 'bridals/'+makeid(14) + "." + i.filename.split('.').pop()
            const newPath = newName
    ...
    if (result.modifiedCount) {
        return res.json({ status: 'success', message: 'Makeup Videos successfully Updated', data: {} })
    } else {
        return res.json({ status: 'error', error: '009', message: 'Something went wrong' })
    }
...
async function videoDelete(req, res) {
    ...
    try {
        fs.unlinkSync('/public/bridals/' + deletedVideo)
        if (result.modifiedCount) {
            return res.json({ status: 'success', message: 'Makeup Videos successfully Updated', data: {} })
        } else {
            return res.json({ status: 'error', error: '009', message: 'Something went wrong' })
